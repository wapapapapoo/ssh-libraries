...
public class ChannelSession extends AbstractServerChannel {
    public static final List<ChannelRequestHandler> DEFAULT_HANDLERS = Collections
            .singletonList(PuttyRequestHandler.INSTANCE);
    ...
    protected boolean mayWrite() {
        ...
        // We need to allow writing while closing in order to be able to flush the
    ...
    public class CommandCloseable extends IoBaseCloseable {
        ...
        public CloseFuture close(boolean immediately) {
            if (immediately || (commandInstance == null)) {
                ...
                ScheduledExecutorService scheduler = Objects.requireNonNull(manager.getScheduledExecutorService(),
    ...
    protected RequestHandler.Result handleShellParsed(String request) throws IOException {
        ...
        ServerFactoryManager manager = Objects.requireNonNull(shellSession.getFactoryManager(),
                "No server factory manager");
        ...
        try {
            ...
            // LoggerFactory.getLogger(this.getClass()).error("error occured: ", e);
    ...
    protected RequestHandler.Result handleExecParsed(
        ...
        ServerFactoryManager manager = Objects.requireNonNull(cmdSession.getFactoryManager(),
                "No server factory manager");
    ...
    protected RequestHandler.Result handleSubsystemParsed(String request, String subsystem) throws IOException {
        ServerFactoryManager manager = Objects.requireNonNull(getServerSession(), "No server session")
                .getFactoryManager();
        Collection<? extends SubsystemFactory> factories = Objects.requireNonNull(manager, "No server factory manager")
     ...
     * will no longer be invoked. If you call this method from
     * {@code Command#start(ChannelSession, Environment)}, the
     * input stream you received in
     * {@link Command#setInputStream(java.io.InputStream)} will not read any data.
     *
    ...
    /**
     * A special {@link ChannelDataReceiver} that can be used to receive data sent
     * as &quot;extended&quot; - usually
     * STDERR. <B>Note:</B> by default any such data sent to the channel session
     * causes an exception, but specific
     * implementations may choose to register such a receiver (e.g., for custom
     * usage of the STDERR stream). A good
     * place in the code to register such a writer would be in commands that also
    ...
    /**
     * Called by {@link #prepareChannelCommand(String, Command)} in order to set up
     * the command's streams, session,
     ...
     *         creation of the command is failed and the original command (if any)
     *         destroyed (eventually).
     *         <B>Note:</B> if a different command instance than the input one is
     *         returned, then it is up to
     *         the implementor to take care of the wrapping or destruction of the
     *         original command instance.
        ...
        if (extendedDataBuffer != null) {
            if (extendedDataWriter == null) {
                throw new UnsupportedOperationException(
                        "No extended data writer available though " + extendedDataBuffer.available()
                                + " bytes accumulated");
            }
